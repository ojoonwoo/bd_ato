...
$media_query	= "SELECT design_media, COUNT( design_media ) media_cnt FROM ".$_gl['design_info_table']." WHERE 1 AND design_regdate LIKE '%".$todayDate."%' GROUP BY design_media";
$media_res		= mysqli_query($my_db, $media_query);

$unique_query	= "SELECT * FROM ".$_gl['design_info_table']." WHERE 1 AND design_regdate LIKE  '%".$todayDate."%' GROUP BY design_ipaddr";
...
{
    $media_name[]	= $media_daily_data['design_media'];
    $media_cnt[]	= $media_daily_data['media_cnt'];
    $pc_query		= "SELECT * FROM ".$_gl['design_info_table']." WHERE 1 AND design_regdate LIKE  '%".$todayDate."%' AND design_media='".$media_daily_data['design_media']."' AND design_gubun='PC' ";
    $pc_count		= mysqli_num_rows(mysqli_query($my_db, $pc_query));
    $mobile_query	= "SELECT * FROM ".$_gl['design_info_table']." WHERE 1 AND design_regdate LIKE  '%".$todayDate."%' AND design_media='".$media_daily_data['design_media']."' AND design_gubun='MOBILE'";
...
// Rename worksheet
$objPHPExcel->getActiveSheet()->setTitle('이브자리_디자인투표_참여자데이터_'.$todayDate);
...
header('Content-Disposition: attachment;filename="이브자리_디자인투표_참여자데이터_'.$todayDate.'.xls"');
